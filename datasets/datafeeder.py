...
class DataFeeder(threading.Thread):
  ...
  def __init__(self, coordinator, metadata_filename, hparams):
    ...
    queue = tf.FIFOQueue(
        8,
        [tf.int32, tf.int32, tf.int32, tf.float32, tf.float32, tf.float32],
        name='input_queue')
    self._enqueue_op = queue.enqueue(self._placeholders)
    self.inputs, self.input_lengths, self.speaker_ids, self.mel_targets, self.linear_targets, self.stop_token_targets = queue.dequeue()
  ...
  def _get_next_example(self):
    ...
    return (input_data, mel_target, linear_target, stop_token_target, len(linear_target), speaker_id)
